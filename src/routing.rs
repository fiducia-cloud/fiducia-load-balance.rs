...
//! Only the path-parsing (`routing_key_from_path`) is LB-specific.
...
mod tests {
    ...
    #[test]
    fn key_extraction() {
        assert_eq!(routing_key_from_path("/v1/kv/orders").as_deref(), Some("orders"));
        assert_eq!(routing_key_from_path("/v1/locks/checkout/info").as_deref(), Some("checkout"));
        assert_eq!(routing_key_from_path("/v1/rw/report/read").as_deref(), Some("report"));
        assert_eq!(routing_key_from_path("/v1/elections/cleanup/campaign").as_deref(), Some("cleanup"));
        assert_eq!(routing_key_from_path("/v1/services/api/instances/i1").as_deref(), Some("api"));
        // keyless / cross-shard
